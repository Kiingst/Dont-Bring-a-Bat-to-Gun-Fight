...
var Common_Upgrade_Dict = { 
	...
	"Jump_Height" = ["res://icon.svg", "Jump_Higher" ,"player.jump_height += 50"],
	"Dodge_Time" = ["res://icon.svg", " Dodge for 0.5 seconds longer" ,"player.invulnerability_time += 0.25"],
...
var Rare_Upgrade_Dict = {
	"Ball_Speed" = ["res://icon.svg", "Ball is thrown faster","player.max_Bullet_Speed += 200" ],
	"Ball_Damage" = ["res://icon.svg", "Ball +1 damage" ,"player.Bullet_Damage += 1"],
	"Ball_No_Player_Damage" = ["res://icon.svg", "Player thrown balls dont hit player","player.Take_Own_Ball_damage = false" ]
}
